...
var client1, client3, client4, client5 *rpc.Client

//var err1, err3, err4, err5 *net.OpError
...
func (a *API) Proposer(empty string, reply *string) error {
	...
	*/
	...
	var sendId = fmt.Sprintf("%d-%d", id, 1)
	...
	if majortiy >= 3 {
		/*
			If majority promises, send accept command messages to servers 1,3,4,5
		*/
		selfAccept("command2")
		client1.Call("API.Accept", "command2", &reply1)
		client3.Call("API.Accept", "command2", &reply3)
		client4.Call("API.Accept", "command2", &reply4)
		client5.Call("API.Accept", "command2", &reply5)
	}
...
func (a *API) Accept(r string, reply *string) error { // r will contain command
	r += "-1" //appending server number to identify the server
	selfLearn(r)
